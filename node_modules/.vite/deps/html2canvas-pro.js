...
var ElementContainer = class {
  constructor(context, element) {
    ...
    if (isHTMLElementNode(element)) {
      if (this.styles.animationDuration.some((duration2) => duration2 > 0)) {
...
var isCustomElement = (node) => node.tagName.indexOf("-") > 0;
...
var createIFrameContainer = (ownerDocument, bounds) => {
...
var createPseudoHideStyles = (body) => {
  createStyles(body, `.${PSEUDO_HIDE_ELEMENT_CLASS_BEFORE}${PSEUDO_BEFORE}${PSEUDO_HIDE_ELEMENT_STYLE}
         .${PSEUDO_HIDE_ELEMENT_CLASS_AFTER}${PSEUDO_AFTER}${PSEUDO_HIDE_ELEMENT_STYLE}`);
};
var createStyles = (body, styles) => {
...
var CacheStorage = class _CacheStorage {
  static getOrigin(url) {
    const link = _CacheStorage._link;
    if (!link) {
      return "about:blank";
    }
    link.href = url;
  ...
  static isSameOrigin(src) {
    return _CacheStorage.getOrigin(src) === _CacheStorage._origin;
  }
  static setContext(window2) {
    _CacheStorage._link = window2.document.createElement("a");
    _CacheStorage._origin = _CacheStorage.getOrigin(window2.location.href);
  }
};
...
var CanvasRenderer = class _CanvasRenderer extends Renderer {
            ...
            if (styles.textDecorationLine.length) {
              ...
              styles.textDecorationLine.forEach((textDecorationLine2) => {
                switch (textDecorationLine2) {
                  case 1:
                    this.ctx.fillRect(text2.bounds.left, text2.bounds.top + text2.bounds.height - decorationLineHeight, text2.bounds.width, decorationLineHeight);
                    break;
                  case 2:
                    this.ctx.fillRect(text2.bounds.left, text2.bounds.top, text2.bounds.width, decorationLineHeight);
                    break;
                  case 3:
                    this.ctx.fillRect(text2.bounds.left, text2.bounds.top + (text2.bounds.height / 2 - decorationLineHeight / 2), text2.bounds.width, decorationLineHeight);
                    break;
                }
              });
            }
          ...
          case 1:
            if (styles.webkitTextStrokeWidth && text2.text.trim().length) {
              ...
              if (styles.letterSpacing === 0) {
                ...
                const letters = segmentGraphemes(text2.text);
                letters.reduce((left, letter) => {
                  this.ctx.strokeText(letter, left, text2.bounds.top + baseline);
                  return left + this.ctx.measureText(letter).width;
                }, text2.bounds.left);
              }
            }
  ...
  async renderNodeContent(paint) {
    ...
    if (isTextInputElement(container) && container.value.length) {
      const [font, fontFamily2, fontSize2] = this.createFontStyle(styles);
  ...
  resizeImage(image2, width, height) {
    ...
    for (const backgroundImage2 of container.styles.backgroundImage.slice(0).reverse()) {
      if (backgroundImage2.type === 0) {
        ...
        if (image2) {
          ...
          const pattern = this.ctx.createPattern(this.resizeImage(image2, width, height), "repeat");
        ...
        const [rx, ry] = calculateRadius(backgroundImage2, x, y, width, height);
        if (rx > 0 && ry > 0) {
...
var fixIOSSystemFonts = (fontFamilies) => {
  return /iPhone OS 15_(0|1)/.test(window.navigator.userAgent) ? fontFamilies.filter((fontFamily2) => iOSBrokenFonts.indexOf(fontFamily2) === -1) : fontFamilies;
};
...
var html2canvas = (element, options = {}) => {
  return renderElement(element, options);
};
if (typeof window !== "undefined") {
  CacheStorage.setContext(window);
}
var renderElement = async (element, opts) => {
